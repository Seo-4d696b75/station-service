...
tree = KdTree(json.load(open('./data/tree.json','r',encoding='utf-8')))
...
async def get_station(
	...
	if code == 0 and id is None:
		raise APIException(400, 'lack of query', 'either code or id must be spesified.')
	if id is not None:
		data = Session().query(Station).filter(Station.id == id).first()
		if data is None:
			raise APIException(404, 'station not found', f"station identified by id:{id} not found")
	else:
...
async def get_line(
	...
	if id is not None:
		...
		if data is None:
			raise APIException(404, 'line not found', f"line identified by id:{id} not found")
	else:
...
async def nearest_search(
	...
	data = list(map(lambda  d: {
		...
		'station': station_schema.dump(
			session.query(Station).filter(Station.code == d['code']).first()
		)
	}, neighbors))
	return data